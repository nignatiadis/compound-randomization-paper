...
# Do a paired test instead


```{r}
idx_naive_act <- which(cellType %in% c("naive", "act_naive"))
```

```{r}
mVals_paired <- mVals[,idx_naive_act]
```

```{r}
targets_paired <- targets[idx_naive_act,]
```
```{r}
# this is the factor of interest
cellType_paired <- factor(targets_paired$Sample_Group)
# this is the individual effect that we need to account for
individual_paired <- factor(targets_paired$Sample_Source)
```

# use the above to create a design matrix
```{r}
design_paired <- model.matrix(~0+cellType_paired+individual_paired, data=targets_paired)
colnames(design_paired) <- c(levels(cellType_paired),levels(individual_paired)[-1])
design_paired
```

```{r}
fit_paired <- lmFit(mVals_paired, design_paired)
```


```{r}
contMatrix_paired <- makeContrasts(naive-act_naive,
...
```{r}
fit2_paired <- contrasts.fit(fit_paired, contMatrix_paired)
fit2_paired <- eBayes(fit2_paired)
```

```{r}
summary(decideTests(fit2_paired))
```
...
```
```{r}
my_df_naive_vs_act_paired <- data.frame(probe = rownames(fit2_paired$coefficients),
                 limma_pvalue = fit2_paired$p.value[,naive_vs_act],  
                 sigma_hat = fit2_paired$stdev.unscaled[,naive_vs_act] * fit2_paired$sigma, 
                 mu_hat = fit2_paired$coefficients[,naive_vs_act])
```
...
```
```{r}
write_csv(my_df_naive_vs_act_paired, file="methylation_naive_vs_act_paired.csv")
```
